...
| `submit_presentation` | beneficiary; `ACTIVE` or cure-eligible `DISCREPANT_OPEN` | `now < presentation_deadline` (and cure deadline); presentation ID one use | nonpayable; evidence/presentation views | wrong caller, schema/digest/origin/replay, boundary tripwire |
| `adjudicate` | permissionless; `PRESENTED` or retryable `UNVERIFIABLE_OPEN` | `now < adjudication_deadline`; attempt nonce one use | no movement unless validated `COMPLIANT`; attempt/discrepancy/accounting views | malformed or malicious output, digest mismatch, duplicate, boundary |
...
| `retry_unverifiable` | beneficiary; `UNVERIFIABLE_OPEN` | `now < retry_deadline`; max 3 attempts | no movement | changed evidence, exhausted retry, caller/state/boundary |
| `expire_credit` | permissionless; live non-entitled states | `now >= final_deadline`; one refund | entire locked purse -> issuer refund credit | early/equal/late, duplicate, after entitlement |
